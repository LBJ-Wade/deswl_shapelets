// $Id: Shear.cpp,v 1.16 2009/10/09 12:37:34 garyb Exp $
...
void Shear::getEta1Eta2(double& eta1, double& eta2) {
  double scale;
  hasMatrix = false;
...
Shear& Shear::operator+=(const Shear& s2) {
  ...
  assert(s1sq<=1.);	//addition requires a realizable shear.
  assert(s2.e1*s2.e1+s2.e2*s2.e2 <= 1.);
...
Ellipse
Ellipse::fromMatrix(const SqDMatrix& m, double& rotation, bool& parityFlip) {
  assert(m.size()==2);
