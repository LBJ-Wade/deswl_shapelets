// 	$Id: Bounds.h,v 1.19 2009/10/06 20:24:29 garyb Exp $	
...
namespace laguerre {
    ...
    public:
        ...
        Array<Bounds<T> > quarter() const {return divide(2,2);}
        Array<Bounds<T> > divide(uint nx, uint ny) const;
        void write(ostream& fout) const
        {
            if (defined) 
    ...
    template <class T>
    Array< Bounds<T> > Bounds<T>::divide(uint nx,uint ny) const
    {
        if (!defined) return Array< Bounds<T> >(nx,ny);
        Array< Bounds<T> > temp(nx,ny);
        ...
        for(uint i=0;i<nx;i++) for(uint j=0;j<ny;j++)
            temp.Set(i,j,Bounds<T>(x[i],x[i+1],y[j],y[j+1]));
        return temp;
    }
