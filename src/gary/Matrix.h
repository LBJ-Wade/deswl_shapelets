// 	$Id: Matrix.h,v 2.23 2009/10/22 16:42:38 garyb Exp $	
...
#define MVAssert(x) if(!(x)) { \
    ...
    public:
        ...
        {
            for(size_t i=0;i<va.size();i++) va[i] = value;
            Changed();
            return *this;
        }
    ...
    protected:
        ...
        { if(svd) delete svd; svd=0; isAltered=false; }
    ...
    public:
        ...
        T& operator[](size_t i) 
        { MVAssert(i<va.size()); return va[i]; }
        T operator[](size_t i) const 
        { MVAssert(i<va.size()); return va[i]; }

        void Set(size_t i,T t)
        { MVAssert(i<va.size()); va[i] = t; }
