...
void makePsi(DVector& psi, std::complex<double> z, int order)
{
    ...
    for(int N=2,k=3;N<=order;++N) {
        ...
        psi.TMV_subVector(k,k+N-1) = rsq * psi.TMV_subVector(k-2*N-1,k-N-2);
        psi.TMV_subVector(k,k+N-1) -= (N-1.) * psi.TMV_subVector(k-2*N-1,k-N-2);
        for(int m=N-2,p=N-1,q=1;m>=0;--p,++q,m-=2) {
            double pq = p*q;
            if (m==0) {
                psi(k) /= sqrt(pq);
                ...
                psi.TMV_subVector(k,k+2) /= sqrt(pq);
                if (q > 1)
                    psi.TMV_subVector(k,k+2) -= 
                        sqrt(1.-(N-1.)/pq)*psi.TMV_subVector(k+2-4*N,k+4-4*N);
                k+=2;
            }
        }
    }
