...
struct EllipseSolver::ESImpl
{
    ...
    ESImpl(
        ...
        bool _fixcen, bool _fixgamma, bool _fixmu);
...
void EllipseSolver::ESImpl::doF3(const DVector& x, DVector& f) const
{
    Assert(x.size() == 5);
    Assert(f.size() == 5);
...
void EllipseSolver::ESImpl::doF2(const DVector& x, DVector& f) const
{
    Assert(x.size() == 5);
    Assert(f.size() == 5);
...
void EllipseSolver::ESImpl::doF1(const DVector& x, DVector& f) const
{
    Assert(x.size() == 5);
    Assert(f.size() == 5);
...
void EllipseSolver::ESImpl::doJ3(
    const DVector& x, const DVector& f, DMatrix& J) const
{
    // bx = T S D b0
    //
    ...
    SetupGx(Gxbig,bxorderp2,bxorder,0.);
...
void EllipseSolver::ESImpl::doJ2(
    const DVector& x, const DVector& f, DMatrix& J) const
{
    //xdbg<<"Start doJ2\n";
    //xdbg<<"x = "<<x<<std::endl;
...
void EllipseSolver::ESImpl::doJ1(
    const DVector& x, const DVector& f, DMatrix& J) const
{
    //xdbg<<"Start doJ1\n";
    //xdbg<<"x = "<<x<<std::endl;
